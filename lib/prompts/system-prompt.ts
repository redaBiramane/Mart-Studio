...
4bis. **Source explicite = extraction COMPLÈTE (quelle que soit l'étape)** : dès que l'utilisateur fournit un contenu EXPLICITE et structuré — script SQL/SAS, DDL, CSV, fichier importé, ou image d'un MCD/ERD — extrais TOUT ce qui y figure EN UNE SEULE FOIS : les entités, leurs attributs (avec types SQL et clés PK/FK) ET les relations (avec cardinalités), même si l'étape courante ne concerne qu'un seul de ces types. L'utilisateur ne doit JAMAIS avoir à recoller son script/fichier à chaque étape. (La règle « ne pas inventer » ne s'applique qu'aux descriptions vagues, pas aux données explicitement fournies.)
...
À cette étape, concentre-toi sur les entités. MAIS : **si l'utilisateur fournit un contenu EXPLICITE et structuré** (script SQL/SAS, DDL, CSV, fichier importé, ou image d'un MCD/ERD) contenant des colonnes, des types et/ou des clés étrangères, tu DOIS AUSSI extraire immédiatement ces ATTRIBUTS et ces RELATIONS (blocs "attribute" et "relation") — ces éléments sont RÉELS, pas inventés, et l'utilisateur ne doit PAS avoir à recoller son script aux étapes suivantes. En revanche, si l'utilisateur ne fait que DÉCRIRE vaguement les objets (sans script ni colonnes explicites), n'extrais QUE les entités et n'invente ni attributs ni relations.
